...
post("[ableton-rack-builder-v2] loaded. Send bang to build.\n");
...
function loadbang() {
  post("[ableton-rack-builder-v2] loadbang received. Send manual bang if patch did not build.\n");
}
...
function build() {
  ...
  var startMsg = p.newdefault(170, 150, "message", "script start");
  ...
  outlet(0, "done");
  post("[ableton-rack-builder-v2] bridge patch created. Click script start, select a Rack, then click bang.\n");
}
...
function safeConnect(patcher, source, sourceOutlet, destination, destinationInlet) {
  try {
    ...
    post("[ableton-rack-builder-v2] connect failed: " + error + "\n");
  }
}
