...
function build() {
  var p = this.patcher;
  if (!p) {
    post("[ableton-rack-builder-v7] no patcher available yet.\n");
    return;
  }
  ...
  var route = p.newdefault(170, 200, "route", "plugin_message_uri");
  ...
  post("[ableton-rack-builder-v7] clean autostart patch created. Wait for node connected, then click toggle.\n");
}
...
function clearEverythingExceptAudioAndSelf(patcher) {
  ...
  for (var i = 0; i < boxes.length; i += 1) {
    try {
      ...
      post("[ableton-rack-builder-v7] remove failed: " + error + "\n");
    }
  }
}
...
function safeConnect(patcher, source, sourceOutlet, destination, destinationInlet) {
  try {
    ...
    post("[ableton-rack-builder-v7] connect failed: " + error + "\n");
  }
}
